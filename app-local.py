"""문서 추출 비교 데모(로컬 실행용) — 폴더에 있는 PDF/DOCX를 실제로 추출해 비교한다.

대상 파일은 고정돼 있지 않다. 이 파일 옆에 놓인 .pdf/.docx 를 그대로 집어오므로,
파일을 갈아끼우면 코드 수정 없이 그 파일들을 비교한다.

실행: streamlit run app-local.py

배포 환경에는 Docling·unstructured 를 돌릴 CPU 가 없다. 여기서 나온 결과를
사이드바의 "결과 JSON 내보내기" 로 굳혀 두면, 뷰어인 `app.py` 가 그 값을 읽어
같은 화면을 그린다.
"""
...
from export_results import build_payload, write_payload
...
# 방금 화면에 그린 결과를 그대로 JSON 으로 굳힌다. 배포된 app.py 는 이 파일만 읽는다.
st.sidebar.divider()
if st.sidebar.button("💾 결과 JSON 내보내기", width="stretch"):
    payload = build_payload(
        {n: opts[n] for n in selected_names}, selected_methods, results
    )
    out = write_payload(payload)
    st.sidebar.success(f"`{out.name}` 저장 완료 ({out.stat().st_size / 1024:.0f} KB)")
    st.sidebar.caption("이 파일을 app.py 와 함께 커밋하면 배포 화면에 그대로 반영됩니다.")
